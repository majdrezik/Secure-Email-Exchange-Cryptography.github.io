...
public class AES {
	...
	public static byte[] ecb_encrypt(byte[] plainText, byte[] key){
		...
		byte[] ecryptedText = new byte[plainText.length + lenght];
		...
		for (i = 0; i < plainText.length + lenght; i++) {
			if (i > 0 && i % 16 == 0) {
				bloc = encryptBloc(bloc);
				System.arraycopy(bloc, 0, ecryptedText, i - 16, bloc.length);
			}
			if (i < plainText.length)
				bloc[i % 16] = plainText[i];
			else{														
